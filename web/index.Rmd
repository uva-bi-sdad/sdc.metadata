...
region_types <- unlist(read_json("https://raw.githubusercontent.com/uva-bi-sdad/sdc.metadata/master/src/data_repo_structure/region_types.json")) %>% paste0(., collapse = ", ")
measure_types <- unlist(read_json("https://raw.githubusercontent.com/uva-bi-sdad/sdc.metadata/master/src/data_repo_structure/measure_types.json")) %>% paste0(., collapse = ", ")
...
data.table(as.character(read_json("https://raw.githubusercontent.com/uva-bi-sdad/sdc.metadata/master/src/data_repo_structure/column_structure.json"))) %>% 
  cbind(c(
    ...
    standards</a>", 
    "Human and machine readable ID, see <a href = '#measure'> see measure format</a>",
...
data_types <-  unlist(read_json("https://raw.githubusercontent.com/uva-bi-sdad/sdc.metadata/master/src/data_repo_structure/data_types.json")) %>% paste0(., collapse = ", ")
...
data.table(as.character(read_json("https://raw.githubusercontent.com/uva-bi-sdad/sdc.metadata/master/src/data_repo_structure/measure_structure.json"))) %>%
  cbind(c(
    "Thematic category or categories for the measure, one or more of of ['Behavioral Health, Substance Use Disorder and Recovery', 'Business Climate', 'Broadband', 'Demographics', 'Education', 'Employment/Workforce Development', 'Health', 'Housing', 'Health Opportunity Index', 'Nutrition and Food Security', 'Transportation']", 
    "Citations for methods used to produce the measure, <a href = '#citations'> see citations format</a>",
    paste0("Type of the data, automatically inferred, one of [", data_types, "]"),
    "Describes how the measure relates to equity, one of ['Status', 'Accessibility', 'Availability', or 'Affordability']",
    "Related shapefile of points with the measure, see <a href = '#layer'> see layer format</a>",
  ...
  datatable(colnames = c("measure_info key", "description"), 
            ...
            escape=FALSE)
```
## column measure description {#measure}
...
data.table(as.character(read_json("https://raw.githubusercontent.com/uva-bi-sdad/sdc.metadata/master/src/data_repo_structure/source_structure.json"))) %>% 
  cbind(c(
...
## measure_info layer description {#layer}
```{r eval = TRUE, echo = FALSE, include = TRUE, message = FALSE, warning = FALSE}
