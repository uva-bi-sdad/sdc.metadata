...
data.table(as.character(read_json("https://raw.githubusercontent.com/uva-bi-sdad/sdc.metadata/master/data/measure_structure.json"))) %>%
  cbind(c(
   ...
   cbind(c(
    ...
    "",
    "")) %>%
  datatable(colnames = c("measure_info key", "Description", "Allowed Values/Format", "Derived From"), 
            options = list("pageLength" = 20),
            escape=FALSE, rownames = FALSE)
```
## measure guidance {#measure}
Measures should be named to balance human and machine-readability. *Generally*, the format for measures should be <code> topic_method </code>. Underscores should be used to separate words in a measure. Measures should be renamed to SDC style guidelines after we have manipulated them. The living list of abbreviations can be found here.  
